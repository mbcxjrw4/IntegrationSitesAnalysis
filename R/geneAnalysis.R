...
option_list <- list(
  ...
  make_option("--isgr", type="character", help="Output RDS")
)
...
gencode.v22.genes <- read.table(cfg$gene_ranges)
...
fraietta_TPM <- read.table(cfg$tpm, sep = "\t", header = T, stringsAsFactors = F)
	
# Oncogene comparisons within expression bins
gencode.v22.geneDF <- merge(data.frame(mcols(gencode.v22.genes)), fraietta_TPM[, c("gene_name", "mean_TPM")], by = "gene_name", all.x = T, all.y = F, sort = F)
...
if (!file.exists(opt$plot)) {
	...
	# combine
	plotObject <- p1 + plot_spacer() + p2 + plot_layout(widths = c(0.75, 0.25, 6))
	# save
	ggsave(opt$plot, plotObject, width = 25, height = 15, units = "cm", dpi = 300)
}
	
# Save IS GRanges object
saveRDS(IS.GR, file = opt$isgr)

message("Gene analysis complete ✅")
